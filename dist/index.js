        ...
        api.registerTool({
            ...
            async execute(_toolCallId, params) {
                try {
                    ...
                    if (!goal)
                        return toToolText("goal is required", { ok: false });
                    const scope = typeof p.scope === "string" && p.scope.trim() ? p.scope.trim() : resolved.scope;
        ...
        api.registerTool({
            ...
            async execute(_toolCallId, params) {
                try {
                    ...
                    const safetyLevel = safetyRaw === "auto_ok" || safetyRaw === "needs_confirm" || safetyRaw === "manual_only"
                        ? safetyRaw
                        : undefined;
            ...
            async execute(_toolCallId, params) {
                try {
                    ...
                    const status = statusRaw === "success" || statusRaw === "failed" || statusRaw === "skipped" || statusRaw === "partial"
                        ...
                        : "";
                    if (!runId || !status)
                        return toToolText("runId and valid status are required", { ok: false });
                    const scope = typeof p.scope === "string" && p.scope.trim() ? p.scope.trim() : resolved.scope;
                    const out = await client.replayStepAfter({
                        ...
                        status,
                        stepId: typeof p.stepId === "string" && p.stepId.trim() ? p.stepId.trim() : undefined,
                        stepIndex: Number.isInteger(p.stepIndex) ? Number(p.stepIndex) : undefined,
                        ...
                        artifactRefs: Array.isArray(p.artifactRefs)
                            ? p.artifactRefs.filter((v) => typeof v === "string" && v.trim().length > 0)
                            : undefined,
            ...
            async execute(_toolCallId, params) {
                try {
                    ...
                    const targetStatus = targetRaw === "draft" || targetRaw === "shadow" || targetRaw === "active" || targetRaw === "disabled"
                        ? targetRaw
                        : "";
                    if (!playbookId || !targetStatus)
                        return toToolText("playbookId and valid targetStatus are required", { ok: false });
                    const scope = typeof p.scope === "string" && p.scope.trim() ? p.scope.trim() : resolved.scope;
                    const out = await client.replayPlaybookPromote({
                        scope,
                        playbookId,
                        targetStatus,
                        fromVersion: Number.isInteger(p.fromVersion) ? Number(p.fromVersion) : undefined,
        ...
        api.registerTool({
            ...
            async execute(_toolCallId, params) {
                try {
                    ...
                    if (!playbookId || !action)
                        return toToolText("playbookId and action are required", { ok: false });
                ...
                try {
                    ...
                    const out = await client.replayPlaybookRun({
                        ...
                        params: p.params && typeof p.params === "object" && !Array.isArray(p.params)
                            ? p.params
                            : undefined,
                        maxSteps: Number.isInteger(p.maxSteps) ? Number(p.maxSteps) : undefined,
                    });
