    ...
    api.registerTool(
      {
        ...
        async execute(_toolCallId: string, params: unknown) {
          try {
            ...
            const scope = typeof p.scope === "string" && p.scope.trim() ? p.scope.trim() : resolved.scope;
            const decisionLimit = Number.isFinite(p.decisionLimit as number) ? Number(p.decisionLimit) : undefined;
            const feedbackLimit = Number.isFinite(p.feedbackLimit as number) ? Number(p.feedbackLimit) : undefined;
            ...
            return toToolText(
              `Policy run loaded (status=${String(lifecycle.status ?? "n/a")}, decisions=${String(lifecycle.decision_count ?? "n/a")})`,
              out,
            );
          } catch (err) {
...
function extractUsageTotalTokens(v: unknown): number | null {
...
function extractUsagePromptTokens(v: unknown): number | null {
  const obj = toRecord(v);
  const usage = toRecord(obj.usage);
  const compileSummary = toRecord(obj.compile_summary);
  const usageEstimate = toRecord(compileSummary.usage_estimate);
...
function extractUsageCompletionTokens(v: unknown): number | null {
  const obj = toRecord(v);
  const usage = toRecord(obj.usage);
  const compileSummary = toRecord(obj.compile_summary);
  const usageEstimate = toRecord(compileSummary.usage_estimate);
...
function extractUsageSource(v: unknown): string | null {
  const obj = toRecord(v);
  const usage = toRecord(obj.usage);
  const compileSummary = toRecord(obj.compile_summary);
  const usageEstimate = toRecord(compileSummary.usage_estimate);
